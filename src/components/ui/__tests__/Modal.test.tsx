...
describe("Modal", () => {
  it("does not render when open=false", () => {
    const { container } = render(<Modal open={false} onClose={vi.fn()}>Hidden</Modal>);
    expect(container.firstChild).toBeNull();
    expect(screen.queryByText("Hidden")).toBeNull();
  });

  it("renders children when open=true", () => {
    render(<Modal open={true} onClose={vi.fn()}>Content here</Modal>);
  ...
  it("renders close button with data-close-modal and aria-label (used by useFocusTrap Escape handler)", () => {
    render(<Modal open={true} onClose={vi.fn()} title="Test">X</Modal>);
  ...
  it("does NOT call onClose when clicking inside the panel (stopPropagation)", () => {
    const onClose = vi.fn();
    render(<Modal open={true} onClose={onClose}><div data-testid="inner">X</div></Modal>);
  ...
  it("applies the size class (sm/md/lg)", () => {
    const { rerender } = render(<Modal open={true} onClose={vi.fn()} size="sm">X</Modal>);
    expect(screen.getByRole("dialog").querySelector(".max-w-sm")).not.toBeNull();
    rerender(<Modal open={true} onClose={vi.fn()} size="lg">X</Modal>);
  ...
  it("removes Escape listener when modal closes (does not call onClose after close)", () => {
    const onClose = vi.fn();
    const { rerender } = render(<Modal open={true} onClose={onClose}>X</Modal>);
    rerender(<Modal open={false} onClose={onClose}>X</Modal>);
    document.dispatchEvent(new KeyboardEvent("keydown", { key: "Escape" }));
    expect(onClose).not.toHaveBeenCalled();
  });
});
