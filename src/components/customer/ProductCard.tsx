...
export function ProductCard({
  ...
  return (
    <Card padded={false} className="flex items-center gap-3 p-3 hover:shadow-md transition-shadow">
      ...
      <div className="shrink-0">
        {qty === 0 ? (
          ...
          <div className="flex items-center gap-1">
            <Button variant="secondary" size="icon" onClick={() => onSetQty(qty - 1)} aria-label={`Diminuir ${product.name}`}>
              <Minus className="w-3 h-3" />
            </Button>
            <span className="w-5 text-center text-xs font-semibold text-ink">{qty}</span>
            <Button variant="primary" size="icon" onClick={() => onSetQty(qty + 1)} aria-label={`Aumentar ${product.name}`}>
