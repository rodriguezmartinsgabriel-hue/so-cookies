                    ...
                    <TBody>
                      {filtered.map((item: Ingredient) => (
                        <Tr key={item.id}>
                          ...
                          <Td className="text-sm font-semibold text-ink text-right">R$ {((item.stockKg || 0) * (item.costPerKg || 0)).toFixed(2)}</Td>
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input type="text" placeholder="Ex: Dona Benta" value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-muted uppercase tracking-wide mb-1.5">Estoque Atual (kg)</label>
                    <Input type="number" step="0.01" placeholder="0" value={form.stockKg} onChange={(e) => setForm({ ...form, stockKg: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-muted uppercase tracking-wide mb-1.5">Estoque Mínimo (kg)</label>
                    <Input type="number" step="0.01" placeholder="0" value={form.minStockKg} onChange={(e) => setForm({ ...form, minStockKg: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-muted uppercase tracking-wide mb-1.5">Custo/kg (R$) *</label>
                    <Input type="number" step="0.001" placeholder="0.00" value={form.costPerKg} onChange={(e) => setForm({ ...form, costPerKg: e.target.value })} />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted uppercase tracking-wide mb-1.5">Fornecedor</label>
                  <Input type="text" placeholder="Nome do fornecedor" value={form.supplier} onChange={(e) => setForm({ ...form, supplier: e.target.value })} />
                </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        {isAdmin && (
          <Button onClick={() => { setEditingTier(null); setForm({ name: "", minQty: "", maxQty: "", price: "", productId: "", type: "assado" }); setShowModal(true); }}>
              ...
              <TBody>
                {congeladoTiers.map((tier: PriceTier) => (
                  <Tr key={tier.id}>
                    ...
                    <Td className="text-sm font-medium text-ink">{tier.name}</Td>
                    <Td className="text-sm text-right text-muted">{tier.minQty}</Td>
                    <Td className="text-sm font-semibold text-ink text-right">R$ {(tier.price * tier.minQty).toFixed(2)}</Td>
      ...
      {showModal && (
        <Modal
          ...
          footer={
            <div className="flex gap-2">
              <Button variant="secondary" className="flex-1" onClick={() => { setShowModal(false); setEditingTier(null); }}>Cancelar</Button>
        ...
        >
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              <Input type="text" placeholder="Ex: Assado 3un" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-muted uppercase tracking-wide mb-1.5">Qtd Mínima</label>
                <Input type="number" placeholder="1" value={form.minQty} onChange={(e) => setForm({ ...form, minQty: e.target.value })} />
              </div>
              <div>
                <label className="block text-xs font-medium text-muted uppercase tracking-wide mb-1.5">Qtd Máxima</label>
                <Input type="number" placeholder="Opcional" value={form.maxQty} onChange={(e) => setForm({ ...form, maxQty: e.target.value })} />
              </div>
            </div>
            <div>
