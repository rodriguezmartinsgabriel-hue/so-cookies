...
export type Profile = {
  ...
  addressCep?: string | null; addressStreet?: string | null;
  addressNumber?: string | null; addressComplement?: string | null;
  addressNeighborhood?: string | null; addressCity?: string | null;
  addressState?: string | null; hasPassword?: boolean;
}

export type PublicOrder = {
  id: string; status: string; total: number; pickupCode: string | null;
  notes: string | null; createdAt: string; deliveryDate: string | null;
  deliveryRouteId: string | null;
  deliveryRoute?: { id: string; name: string; windowStart?: string; windowEnd?: string } | null;
  deliveryAddress: string | null; deliveryCep: string | null;
  deliveryStreet: string | null; deliveryNumber: string | null;
  deliveryComplement: string | null; deliveryNeighborhood: string | null;
  deliveryCity: string | null; deliveryState: string | null;
  items: PublicOrderItem[];
}

export const statusLabel: Record<string, string> = {
  PENDENTE: "Recebido", CONFIRMADO: "Confirmado",
  PRODUCAO: "Em produção", PRONTO: "Pronto",
  ENTREGA: "Em entrega", CONCLUIDO: "Finalizado",
  CANCELADO: "Cancelado",
}
