...
export function getHostRole(hostname: string | undefined): HostRole {
  ...
  const storeHost = normalizeHost(process.env.STORE_HOST ?? process.env.NEXT_PUBLIC_STORE_HOST)
  const staffHost = normalizeHost(process.env.STAFF_HOST ?? process.env.NEXT_PUBLIC_STAFF_HOST)
