...
async function createSaleForOrder(
  ...
  const saleItems = order.items.filter((item): item is { productId: string; qty: number; price: number } =>
...
export async function updateOrder(
  ...
  if ("deliveryDate" in data)
    updateData.deliveryDate = data.deliveryDate ? new Date(`${data.deliveryDate}T00:00:00.000Z`) : null
...
export async function updateCashEntry(
  id: string,
  data: Partial<{ type: "ENTRADA" | "SAIDA"; category: string; description: string; amount: number; date: string }>,
) {
  const updateData: Record<string, unknown> = {}
  if (data.type) {
...
export async function updateProduction(
  id: string,
  data: Partial<{ status: string; endTime: string; notes: string; qty: number }>,
) {
  const updateData: Record<string, unknown> = {}
  if (data.status) updateData.status = data.status
...
export async function createDeliveryCost(data: {
  channel: string
  amount: number
  orderId?: string
  notes?: string
...
export async function updateDeliveryCost(
  id: string,
  data: Partial<{ channel: string; amount: number; orderId: string; notes: string; date: string }>,
) {
...
export async function findOrCreateContact(
  ...
  const existing = or.length > 0 ? await db.contact.findFirst({ where: { OR: or }, include }) : null
